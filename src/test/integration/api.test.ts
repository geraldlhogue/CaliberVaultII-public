...
// Ensure supabase has from/channel mocks and is visible globally
...
describe('API Integration Tests', () => {
  ...
  beforeEach(() => {
    ...
    // Reset global supabase and its methods for each test
    (globalThis as any).supabase = supabase;
    (supabase as any).from = vi.fn();
    (supabase as any).channel = vi.fn();
  });
  ...
  describe('REST API Endpoints', () => {
    ...
    it('handles API errors gracefully', async () => {
      const mockChain = {
        select: vi.fn().mockReturnThis(),
        // Instead of rejecting the whole chain, resolve with an error object
        then: (onFulfilled: any) =>
          Promise.resolve({
            data: null,
