...
export const MountingTypeManager: React.FC = () => {
  ...
  const [formData, setFormData] = useState({ name: '', thread_pitch: '', description: '' });

  useEffect(() => { fetchItems(); }, []);

  const fetchItems = async () => {
    const { data } = await supabase.from('mounting_types').select('*').order('name');
  ...
  const filtered = items.filter(i => 
    ...
    i.thread_pitch?.toLowerCase().includes(search.toLowerCase())
  );
    ...
    <div className="space-y-4">
      <form onSubmit={handleSubmit} className="bg-slate-800 p-4 rounded-lg space-y-3">
        <div className="grid grid-cols-3 gap-3">
          ...
          <input placeholder="Thread Pitch (e.g., 5/8x24)" value={formData.thread_pitch}
            onChange={(e) => setFormData({...formData, thread_pitch: e.target.value})}
            className="bg-slate-700 text-white px-3 py-2 rounded" />
        ...
        <button type="submit" className="bg-yellow-600 hover:bg-yellow-700 text-white px-4 py-2 rounded">
          {editing ? 'Update' : 'Add'} Mounting Type
        </button>
        {editing && <button type="button" onClick={() => { setEditing(null); setFormData({ name: '', thread_pitch: '', description: '' }); }}
          className="ml-2 bg-slate-600 hover:bg-slate-700 text-white px-4 py-2 rounded">Cancel</button>}
      </form>

      <div className="relative">
        <Search className="absolute left-3 top-3 text-slate-400" size={20} />
        <input placeholder="Search mounting types..." value={search} onChange={(e) => setSearch(e.target.value)}
        ...
        <table className="w-full">
          <thead className="bg-slate-700">
            <tr>
              ...
              <th className="text-left p-3 text-slate-300">Thread Pitch</th>
          ...
          <tbody>
            {filtered.map(item => (
              <tr key={item.id} className="border-t border-slate-700 hover:bg-slate-750">
                ...
                <td className="p-3 text-slate-300">{item.thread_pitch || '-'}</td>
