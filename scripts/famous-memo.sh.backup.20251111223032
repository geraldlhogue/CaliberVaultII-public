...
if [ "$tests_failed" -eq 0 ] && [ "$tests_passed" -eq 0 ]; then tests_passed=$(grep -E "^[[:space:]]*✓ " "$work_log" | wc -l | tr -d " "); tests_failed=$(grep -E "^[[:space:]]*× " "$work_log" | wc -l | tr -d " "); fi
if [ "$files_failed" -eq 0 ] && [ "$files_passed" -eq 0 ]; then files_passed="$tests_passed"; files_failed="$tests_failed"; fi
prev_stats="test-artifacts/prev.stats"
prev_tests_failed=""
if [ -f "$prev_stats" ]; then prev_tests_failed=$(grep "^tests_failed=" "$prev_stats" | cut -d"=" -f2 || echo ""); fi
if [ -n "$prev_tests_failed" ]; then if [ "$tests_failed" -lt "$prev_tests_failed" ]; then delta=$((prev_tests_failed - tests_failed)); status="**Status:** Progress — $delta fewer failing tests than previous run."; elif [ "$tests_failed" -gt "$prev_tests_failed" ]; then delta=$((tests_failed - prev_tests_failed)); status="**Status:** Regression — $delta more failing tests than previous run."; else status="**Status:** No change — failing test count unchanged."; fi; else status="**Status:** Baseline — no prior run to compare."; fi
{
  ...
  echo "1) InventoryOperations: align export/import (named vs default)."
  echo "2) StorageService auth: ensure client has auth.getSession and onAuthStateChange (same instance as harness)."
  echo "3) useSubscription mock: set tier/limits and hasFeature() to match assertions."
  echo "4) Integration builders: implement .insert() returning { data, error: null }."
  echo "5) API mocks: getAll()->array, create()->object, error->reject; add client.channel().on().subscribe()."
  echo "6) SmartInstallPrompt: raise BeforeInstallPromptEvent and call prompt()."
  echo "7) Barcode cache: use fake timers or lower delays under NODE_ENV=test."
  echo "8) ErrorHandler API: export names to match test imports."
  echo
  echo "Harness Policy"
  echo "vitest.setup.ts is not frozen. Changes are allowed with a short what/why note; regressions will trigger rollback to the prior harness."
} > "$out"
